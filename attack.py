...
class PGD(Attack):
    ...
    # def __call__(self, embeddings, attention_mask, labels):
    #    training_mode = self.model.training
    #    if self._training_mode:
    #        self.model.train()
    #    else:
    #        self.model.eval()
    #    embeddings = self.forward(embeddings, attention_mask, labels)
    #    if training_mode:
    #        self.model.train()
       
    #    if self._return_type == 'int':
    #        embeddings = self._to_uint(embeddings)
    #    return embeddings
    ...
    def forward(self, embeddings, attention_mask, labels):
        ...
        labels = self._transform_label(embeddings, labels)
        ...
        if self.random_start:
            ...
            adv_embeddings = adv_embeddings + torch.empty_like(adv_embeddings).uniform_(-self.eps, self.eps)
            #adv_embeddings = torch.clamp(adv_embeddings, min=0, max=1).detach()
