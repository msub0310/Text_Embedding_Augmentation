...
#augment params
START_TARGET_EPOCH = 2
MAX_TARGET_EPOCHS = 29
...
for epoch in range(EPOCHS):
    ...
    for TARGET_EPOCH in range(MAX_TARGET_EPOCHS):
        TARGET_EPOCH = TARGET_EPOCH + START_TARGET_EPOCH

        for TARGET_DATA_INDEX in range(1, 11):
            TARGET_DATA_INDEX = float(TARGET_DATA_INDEX)

            print(f"\nTARGET_EPOCH : {TARGET_EPOCH}, TARGET_INDEX : {TARGET_DATA_INDEX}")
    
            load_path = f"E:/adversarial_training_as_augmentation/experiments/results/PGD_BATCH64_EPS0.01_ALPHA0.000125_STEP2_gener/adversarial_examples/epoch_{TARGET_EPOCH}"
    
            with open(load_path + f"/adversarial_examples_{TARGET_DATA_INDEX}.pickle", "rb") as f:
                adversarial_embeddings = pickle.load(f)
                adversarial_embeddings = torch.tensor(adversarial_embeddings, dtype=torch.float)                
    
            with open(load_path + f"/adversarial_labels_{TARGET_DATA_INDEX}.pickle", "rb") as f:
                adversarial_labels = pickle.load(f)
                adversarial_labels = torch.tensor(adversarial_labels, dtype=torch.long)
            
            adversarial_masks = generate_adversarial_masks(model, adversarial_embeddings, device)
    
            adv_train_data_loader = create_adv_loader(adversarial_embeddings, adversarial_labels, adversarial_masks, max_len=MAX_LEN, batch_size=BATCH_SIZE, shuffle=True)            
    
            train_acc, train_loss = train_adv_embedding(
                model,
                adv_train_data_loader,
                loss_fn,
                optimizer,
                device,
                scheduler,
                len(adversarial_labels),
                ACCUMULATION_STEPS
            )
